...
prep.data.kings <- function(rawdata, levels_l){
  ...
  for(i in 1:length(levels_l)){
    ...
    pd2[,i] <- zoo::na.aggregate(object = zoo(pd2[,i], order.by = days_data), by=wday)
  }
...
prep.save.kings <- function(rawdata, levels_l, overwrite=F, name="data.csv"){
  preppy <- prep.data.kings(rawdata, levels_l)
...
prep.save.kings(read.csv("daily_cromwell.csv"), levels_l = c("CO", "NO", "NO2", "PM10", "SO2"),
                overwrite = T, name = "daily_cromwell_2000_2017.csv")
prep.save.kings(read.csv("daily_bloomsbury.csv"), levels_l = c("O3", "CO", "NO", "NO2", "PM10", "SO2"),
