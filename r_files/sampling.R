...
for(h in horizons_vines){
  mat_res[k,] <- tron_air_pollution[[h]]$mean[1,]
  k <- k + 1
}
...
plot(vines_of_interest, edge.labels=c('family-par'), 
     type= 1, cex=1.3, lty=5, interactive=F, col='red')
...
db_unif <- vapply(daily_bloom[,conditional_on], 
                  function(x){
                    if(x <= q_extremes[conditional_on]){ecdf_without[[conditional_on]](x)}
                    else{
                  min(0.9999,ecdf_without[[conditional_on]](q_extremes[conditional_on]) + 0.05*(1-(1+sign(alpha)*(x-q_extremes[conditional_on])/sd_epd[conditional_on]/(beta+kappa))^{-alpha}))
    }}, FUN.VALUE = 1)
...
q_extremes
q_extremes_cond <- vapply(1:ncol(daily_bloom), function(x){mean(cond_data[,x] <= q_extremes[x])}, FUN.VALUE = 1)
q_extremes_cond

extreme_in_daily <- which(daily_bloom[,conditional_on] > q_extremes[conditional_on])
ssmple <- RVineCondSim(target = conditional_on, samples = db_unif[extreme_index[1:100]], RVM = v_temp, nsub=500)
# which(rlexceed(alpha, beta, kappa, rho, times = 1:10000, marg.dist = 'gamma', trawl.function = 'exp', transformation = F, n = 1)>0) %>% length

for(var in 1:ncol(daily_bloom)){
  counter <- 0
  counter_f <- 0
  counter_t <- 0
  ...
  cat('Extreme in', colnames(daily_bloom)[var], '\t h =', h_selected, ' \n')
